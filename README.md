...
数据库文件默认位于项目根目录 `data.db`，可通过环境变量 `DB_PATH`
指定到其他位置（相对路径按项目根目录解析）：

```bash
DB_PATH=/var/lib/swap-station/data.db python run.py
```

种子初始化按稳定业务标识（站点名、车牌、车辆+站点+换电时间）逐条
补齐，重复启动或多进程同时启动都不会产生重复数据；从不完整备份恢复
（例如只剩站点表）后重启，会只补齐缺失的车辆和换电记录，已有站点及
其配置保持原样。
